...
class User 
{
	...
	public function __construct() 
	{
		...
		$this->oauth_secret = $CI->input->cookie($config . "oauthsecret", TRUE);
 		...
 		$select = "users.`ID`, users.`username`, users.`email`,	users.first_name, users.last_name, users.`online_timestamp`, users.avatar, users.email_notification, users.aboutme, users.points, users.premium_time, users.active, users.activate_code, users.user_role, user_roles.name as ur_name, user_roles.admin, user_roles.admin_settings, user_roles.admin_members, user_roles.admin_payment,	user_roles.ID as user_role_id";
	...
	{
		$CI =& get_instance();
		$CI->db->where("ID", $userid)->update("users", array("online_timestamp" => time()));
	}

 ...
 */
	...
	if ( ! function_exists('currency'))
	{
		...
		function currency($val='',$def=''){
			...
			$currency_format = $CI->db->get_where('settings', array('type' => 'currency_format'))->row()->description;
			$symbol_format = $CI->db->get_where('settings', array('type' => 'symbol_format'))->row()->description; 
			$no_of_decimal = $CI->db->get_where('settings', array('type' => 'no_of_decimals'))->row()->description;
			...
			if($currency_id = $CI->session->userdata('currency')){} else {
				$currency_id = $CI->db->get_where('settings', array('type' => 'default_currency'))->row()->description;
			}
			if($def == 'def'){
				$currency_id = $CI->db->get_where('settings', array('type' => 'default_currency'))->row()->description;
			}			
	...
	if ( ! function_exists('translate'))
	{
		function translate($word){
			...
			if($set_lang = $CI->session->userdata('language')){} else {
				// admin decide front side defult language 
				$set_lang = 'spanish';
				// $set_lang = $CI->db->get_where('general_settings',array('type'=>'language'))->row()->value;
			}
			$return = '';
			$result = $CI->db->get_where('language',array('phrase'=>$word));
			if($result->num_rows() > 0){

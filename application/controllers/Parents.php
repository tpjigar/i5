...
class Parents extends CI_Controller
{
    ...
    /****VIEW TESTMARKS*****/
    function view_test_marks($student_id= '',$class_id = '')
    {
        if ($this->session->userdata('parent_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['class_id'] > 0 ) {
                redirect(base_url() . 'index.php?parents/view_test_marks/'.$page_data['student_id'].'/' . $page_data['class_id']  , 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose test, class and subject');
                redirect(base_url() . 'index.php?parents/view_test_marks/'.$page_data['student_id'], 'refresh');
            }
        }
    ...
    /****VIEW EXAM MARKS*****/
    function view_exam_marks($student_id= '',$class_id = '')
    {
        if ($this->session->userdata('parent_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['class_id'] > 0) {
                redirect(base_url() . 'index.php?parents/view_exam_marks/'.$page_data['student_id'].'/' . $page_data['class_id'], 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose exam, class and subject');
                redirect(base_url() . 'index.php?parents/view_exam_marks/'.$page_data['student_id'], 'refresh');
            }
        }
        $page_data['class_id']   = $class_id;
        $page_data['student_id']   = $student_id;

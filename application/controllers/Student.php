...
class Student extends CI_Controller
{
    ...
    function dashboard()
    {
        ...
        $page_data['student_details'] = $this->db->get_where('student',array('student_id' => $this->session->userdata('student_id')
        ))->result_array();
		
        $page_data['notices']    = $this->db->get('noticeboard')->result_array();
         $page_data['invoices']   = $this->db->get_where('invoice', array('student_id' => $this->session->userdata('student_id') ))->result_array();
         $page_data['payments']   = $this->db->get_where('payment', array('student_id' => $this->session->userdata('student_id') ))->result_array();
   ...
   /****MANAGE TEACHERS*****/
    ...
    /****VIEW TESTMARKS*****/
    function view_test_marks($class_id = '')
    {
        if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['class_id'] > 0 ) {
                redirect(base_url() . 'index.php?student/view_test_marks/' . $page_data['class_id']  , 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose test, class and subject');
                redirect(base_url() . 'index.php?student/view_test_marks/', 'refresh');
            }
        }
    ...
    /****VIEW EXAM MARKS*****/
    function view_exam_marks($class_id = '')
    {
        if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['class_id'] > 0) {
                redirect(base_url() . 'index.php?student/view_exam_marks/' . $page_data['class_id'], 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose exam, class and subject');
                redirect(base_url() . 'index.php?student/view_exam_marks/', 'refresh');
            }
        }
        $page_data['class_id']   = $class_id;
     ...
     /****MANAGE TEST MARKS*****/
    function test_marks($test_id = '', $class_id = '', $subject_id = '')
    {
        if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['test_id'] > 0 && $page_data['class_id'] > 0 && $page_data['subject_id'] > 0) {
                redirect(base_url() . 'index.php?student/test_marks/' . $page_data['test_id'] . '/' . $page_data['class_id'] . '/' . $page_data['subject_id'], 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose exam, class and subject');
                redirect(base_url() . 'index.php?student/test_marks/', 'refresh');
            }
        }
    ...
    function document($param1 = '', $param2 = '')
    {
        if ($this->session->userdata('student_login') != 1)
            redirect('login', 'refresh');
        ...
        $data['file_type'] 	= strstr($_FILES["file_name"]["name"],".");
        $data['student_id'] 	= $this->session->userdata('student_id');
        
        $this->db->insert('document',$data);
        
        $document_id            = $this->db->insert_id();
        move_uploaded_file($_FILES["file_name"]["tmp_name"], "uploads/document/".$this->session->userdata('roll')."/" . $_FILES["file_name"]["name"]);
            $this->session->set_flashdata('flash_message' , get_phrase('document_uploaded_successfuly'));
            redirect(base_url() . 'index.php?student/dashboard' , 'refresh');
        }
        if ($param1 == 'do_update') {
             $data['title'] 		= $this->input->post('title');
        ...
        $old_file 	= $this->input->post('old_file');
            ...
            $this->db->update('document', $data);
             move_uploaded_file($_FILES["file_name"]["tmp_name"], "uploads/document/".$this->session->userdata('roll')."/" . $_FILES["file_name"]["name"]);
          if($data['file_name']!=""){
        		unlink("uploads/document/".$this->session->userdata('roll')."/".$old_file);
        	}
        	 $this->session->set_flashdata('flash_message' , get_phrase('document_uploaded_successfuly'));
            redirect(base_url() . 'index.php?student/dashboard/', 'refresh');
        } else if ($param1 == 'edit') {
            $page_data['edit_data'] = $this->db->get_where('document', array(
                'document_id' => $param2
            ))->result_array();
        }
        if ($param1 == 'delete') {

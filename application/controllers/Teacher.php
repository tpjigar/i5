...
class Teacher extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->crud_model->current_academic_year(); 
        /*cache control*/
    ...
    function get_course_classes($course_id)
    {
        $classes = $this->db->get_where('class' , array(
            'course_id' => $course_id,'teacher_id' => $this->session->userdata('teacher_id')))->result_array();
    ...
    /****MANAGE TEST MARKS*****/
    function test_marks($test_id = '', $class_id = '', $subject_id = '',$course_id='')
    {
        if ($this->session->userdata('teacher_login') != 1)
            redirect(base_url(), 'refresh');
        
        if ($this->input->post('operation') == 'selection') {
            $page_data['test_id']    = $this->input->post('test_id');
            $page_data['class_id']   = $this->input->post('class_id');
            $page_data['subject_id'] = $this->input->post('subject_id');
            $page_data['course_id'] = $this->input->post('course_id');
            
            if ($page_data['test_id'] > 0 && $page_data['class_id'] > 0 && $page_data['subject_id'] > 0) {
                redirect(base_url() . 'index.php?teacher/test_marks/' . $page_data['test_id'] . '/' . $page_data['class_id'] . '/' . $page_data['subject_id'] . '/' .  $page_data['course_id'] , 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose test, class and subject');
                redirect(base_url() . 'index.php?teacher/test_marks/', 'refresh');
            }
        }
        if ($this->input->post('operation') == 'update') {
            ...
            $this->db->where('test_mark_id', $this->input->post('test_mark_id'));
            $this->db->update('test_mark', $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?teacher/test_marks/' . $this->input->post('test_id') . '/' . $this->input->post('class_id') . '/' . $this->input->post('subject_id'). '/' . $this->input->post('course_id'), 'refresh');
        }
        ...
        $page_data['page_info'] = 'Test marks';
        
        $page_data['page_name']  = 'test_marks';
        $page_data['page_title'] = get_phrase('manage_test_marks');
        $this->load->view('backend/index', $page_data);
    }
    
    function get_subject_tests($subject_id)
    {
    	 $tests = $this->db->get_where('test' , array(
     ...
     /****VIEW TESTMARKS*****/
    function view_test_marks($class_id = '', $section_id = '', $subject_id = '',$course_id='')
    {
        if ($this->session->userdata('teacher_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['course_id'] > 0 && $page_data['class_id'] > 0 && $page_data['section_id'] > 0) {
                redirect(base_url() . 'index.php?teacher/view_test_marks/' . $page_data['class_id'] . '/' . $page_data['section_id'] .'/' . $page_data['subject_id'] . '/' .  $page_data['course_id'] , 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose test, class and subject');
                redirect(base_url() . 'index.php?teacher/view_test_marks/', 'refresh');
            }
        }
        ...
        $page_data['page_name']  = 'view_test_marks';
        $page_data['page_title'] = get_phrase('view_test_marks');
        $this->load->view('backend/index', $page_data);
    }
    
    
    /****VIEW EXAM MARKS*****/
    function view_exam_marks($class_id = '', $section_id = '', $subject_id = '',$course_id='',$exam_id='')
    {
        if ($this->session->userdata('teacher_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['course_id'] > 0 && $page_data['class_id'] > 0 && $page_data['section_id'] > 0) {
                redirect(base_url() . 'index.php?teacher/view_exam_marks/' . $page_data['class_id'] . '/' . $page_data['section_id'] .'/' . $page_data['subject_id'] . '/' .  $page_data['course_id']. '/' . $page_data['exam_id'], 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose exam, class and subject');
                redirect(base_url() . 'index.php?teacher/view_exam_marks/', 'refresh');
            }
        }
        $page_data['section_id']    = $section_id;
        $page_data['class_id']   = $class_id;
        ...
        $page_data['page_name']  = 'view_exam_marks';
    ...
    /****MANAGE EXAM MARKS*****/
    function marks($exam_id = '', $class_id = '', $subject_id = '',$course_id='')
    {
        if ($this->session->userdata('teacher_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0 && $page_data['subject_id'] > 0) {
                redirect(base_url() . 'index.php?teacher/marks/' . $page_data['exam_id'] . '/' . $page_data['class_id'] . '/' . $page_data['subject_id'] . '/' .  $page_data['course_id'], 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose exam, class and subject');
        ...
        if ($this->input->post('operation') == 'update') {
            ...
            redirect(base_url() . 'index.php?teacher/marks/' . $this->input->post('exam_id') . '/' . $this->input->post('class_id') . '/' . $this->input->post('subject_id') . '/' . $this->input->post('course_id'), 'refresh');
        }
        $page_data['exam_id']    = $exam_id;
        $page_data['class_id']   = $class_id;
        $page_data['subject_id'] = $subject_id;
    ...
    function get_subject_exams($subject_id)
    {
    	 $exams = $this->db->get_where('exam' , array(
            'subject_id' => $subject_id ))->result_array();echo "hi";
    ...
    function class_routine($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        // detect the first section
        $page_data['section_id'] = $param2;
        if ($param2 == ''){
        	$this->db->where('class_id',$param1);
            	$page_data['section_id']     =   $this->db->get('section')->first_row()->class_id;
            }
